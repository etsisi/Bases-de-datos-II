...
[a-zA-Z][a-zA-Z0-9_]*
```
Es decir, tienen que empezar por una letra y, a partir del segundo carácter pueden ser letras, números o el guion bajo

Son case-insensitive, como ya se dijo, salvo que vayan entrecomillados (`quoted identifier`)
...
Las **palabras reservadas** no se pueden usar como identificadores, pero se pueden crear identificadores con nombre de palabra reservada con dobles comillas

```sql
"AUTHORIZE"   "SELECT"    "TABLE"
```

Las **palabras no reservadas** sí se pueden usar como identificadores sin comillas.

[Palabras clave de CQL](https://cassandra.apache.org/doc/latest/cassandra/cql/appendices.html#appendix-A)
...
- `blob` Bytes
- `timeuuid` Versión 1 de UUID
-  `uuid`: Identificador único universal. Cualquier versión.
...
# Creación de `Keyspaces`: Replication
...
# Creación de `Keyspaces`: Replication (2)
...
# Creación de `Keyspaces`: Replication (3)
...
**CUIDADO**: Si se intenta crear un factor de replicación mayor al número de nodos, dará un aviso
...
Es similar al comando `USE` en sistemas relacionales
...
Una clave primaria tiene dos partes
...
---
# Particiones en tablas y `STATIC`
...
Sirve para insertar filas en una tabla.
